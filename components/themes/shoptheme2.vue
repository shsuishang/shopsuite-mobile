	...
	<view class="m-muyin-ad-content">
		<view v-if="(commonTPL.AdContent.F1ProductContents.length>0 || commonTPL.AdContent.F2ProductContents.length>0)">
			<view class="m-muyin-ad-ba-list">
				<navigator v-for="(item, i) in commonTPL.AdContent.F2ProductContents"  :key="i" :url="'/pagesub/product/detail?is_store_flag=' + (commonTPL.isStoreFlag) + '&pid=' + (item.ProductContentID)"
			...
			<view class="m-product-list m-listv">
				<navigator v-for="(item, i) in commonTPL.AdContent.F1ProductContents"  :key="i" :url="'/pagesub/product/detail?is_store_flag=' + (commonTPL.isStoreFlag) + '&pid=' + (item.ProductContentID)"
