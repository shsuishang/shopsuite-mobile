  ...
  <view class="page">
    <view class="m-cells m-cells-form">
      <view class="m-cell">
        ...
        <view class="m-cell-bd" style="display: inherit;">
          ...
          <block v-else-if="langSetComplete">
            <!-- 国家区号 -->
            <picker mode="selector" @change="onIntlChange" :value="mode_index" :range="langLists" range-key="name"
                    style="width: 80rpx;text-align: center;">
              <view class="country-int-code" style="color:#000000">{{ user_intl }}</view>
            </picker>

            <input class="u-input" type="number" style="width: 400rpx;height: 48rpx;" v-model="Phone"
      ...
      <view class="m-cell">
        ...
        <view class="m-cell-bd"><input class="u-input" type="text" v-model="password"
                                       :placeholder="__('可在其它终端通过手机及密码登录')"
      ...
      <view class="m-cell m-cell-vcode">
        ...
        <view class="m-cell-bd"><input class="u-input" type="number" :placeholder="__('请输入验证码')" maxlength="6"
                                       @input="inputcode"/></view>
...
export default {
  ...
  async onLoad(options) {
    ...
    if (!this.userInfo.user_mobile) {
      that.$.confirm("请先绑定手机!", function (e) {
        if (e.confirm) {
          that.$.navigateTo({
            url: "/member/member/bindphone"
          })
        }
      })
    }
    ...
    submitdata: function () {
      ...
      if (that.$.isNull(this.Phone))
        ...
        if (false && this.Phone == this.userInfo.user_mobile) {
          ...
          var params = {
            mobile: that.user_intl + this.Phone,
            verify_key: that.user_intl + this.Phone,
          ...
          that.$.request({
            ...
            success: function (data, status, msg, code) {
              if (status == 200) {
                let user_info = {};
                user_info['user_mobile'] = that.Phone;
                user_info['as'] = 1;
